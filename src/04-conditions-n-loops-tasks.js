...
function getSumBetweenNumbers(n1, n2) {
  const array = Array(n2 - n1 + 1).fill().map((_, i) => n1 + i);
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const XOverlap = valueInRange(rect1.top, rect2.top, rect2.top + rect2.width)
    || valueInRange(rect2.top, rect1.top, rect1.top + rect1.width);
  const YOverlap = valueInRange(rect1.left, rect2.left, rect2.left + rect2.height)
...
function isInsideCircle(circle, point) {
  ...
  return (point.x - centerPoint.x) ** 2 + (point.y - centerPoint.y) ** 2 < circle.radius ** 2;
}
...
function findFirstSingleChar(str) {
  return str.split('').filter((ch, i, obj) => obj.indexOf(ch) === obj.lastIndexOf(ch)).shift();
}
...
function getDigitalRoot(num) {
  const dr = num.toString().split('').reduce((acc, val) => acc + Number(val), 0).toString();
  if (dr.length > 1) {
...
function getMatrixProduct(m1, m2) {
  ...
  const productMatrix = new Array(m1NumRows);
  if (Array.isArray(m1) && Array.isArray(m2) && m1NumRows && m2NumRows) {
    if (m1NumCols === m2NumRows) {
      for (let i = 0; i < m1NumRows; i += 1) {
        productMatrix[i] = new Array(m2NumCols);
...
function evaluateTicTacToePosition(position) {
  ...
  const Oindexes = flatted.map((e, i) => (e === '0' ? i : -1))
    ...
    .split(',')
    .join('');
  const Xindexes = flatted.map((e, i) => (e === 'X' ? i : -1))
    .filter((x) => x >= 0).toString()
    .split(',')
    .join('');
  let result = '';
